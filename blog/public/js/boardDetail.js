...
function get_detail() {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      // 제목 & 작성자 출력
      $('#title_and_author')
        .append(`<h1><input id='title' value="${detail['title']}" type='text' placeholder='제목' ${article_status}></input></h1>                                              
      ...
      $('#content_detail').append(
        `<h5><textarea id='content' ${article_status}>${detail['content']}</textarea></h5>`
      );
      ...
      if (currentUser == detail['userId']) {
        ...
        if (modify_check == 'true') {
          $('#btn').append(
            ` <button id=password-1 onclick='modify_post()' type="button" class="btn btn-outline-warning">저장하기</button>`
          );
          $('#background').css('background-color', '#FCFCEE');
          // 상세 페이지에서는 '수정하기', '삭제하기' 버튼 출력
        } else {
          $('#btn')
            .append(` <button id='password-1' onclick='modify_post()' type="button" class="btn btn-outline-secondary">수정하기</button>
