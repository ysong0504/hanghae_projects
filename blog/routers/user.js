...
const postUserSchema = Joi.object({
    userId: Joi.string().regex(/(?=.*\d)(?=.*[a-z])/).min(3).required(),
...
router.post('/auth', async(req, res) => {
    try {
        const { userId, password } = 
            await postUserAuth.validateAsync(req.body);
        ...
        const token = jwt.sign({ userId: user.userId }, 'fairy-seunghyunkim-not-the-tutor-but-my-bf')
